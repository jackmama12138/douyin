...
import {getHotList,getScoresList} from '../api/ranklist.js'
import AnchorGiftLists from "./AnchorGiftLists.vue";
import {io} from 'socket.io-client'
const socket = io('http://101.43.7.52:4000')

let hotLists = ref([])
let scoreLists = ref([])
let roomId = ref('')
let anchorName = ref('')
const emits = defineEmits(['anchorMessage'])

const toAnchorGiftListPage = (res) => {
  if (!res) return

  // console.log(res)
  emits('anchorMessage', res)
}
...
onMounted(async ()=>{
  const h =  await getHotList()
  // console.log(h)
  hotLists.value = h.data
  const s = await getScoresList()
  scoreLists.value = s.data
})
    ...
    <div class="-item-content-box">
      <a-list max-height="1050px" class="live-rank-list" :bordered="false" :split="false">
        <a-list-item v-for="(idx,key) in hotLists" :key="idx" style="padding: 1px 0px 0px 1px">
          <div class="live-rank-list-item" @click="toAnchorGiftListPage(idx)">
